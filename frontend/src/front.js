...
class App extends Component {
  ...
  handleChangeTopic(t, i) {
    ...
    this.state.chosenTopics[i] = t.trim().replace(" ", "_").toLowerCase();
    this.setState({
      chosenTopics: this.state.chosenTopics
    });
  }
  ...
  render() {
    return (
      <div className="app">
        ...
        <ChoosingTopicsSlide handleChoosingTopic={(t, i) => this.handleChangeTopic(t, i)} class={this.state.topicsSlideClass} topics={this.state.topics} chosenTopicIndexes={[0, -1]} handleStart={() => this.handleStart()} />
        <Settings topics={this.state.topics} messageSpeed={parseInt(this.state.messageSpeed) / 1000} chosenTopics={this.state.chosenTopics} setMessageSpeed={(s) => this.setMessageSpeed(s)} handleChangeTopic={(t, i) => this.handleChangeTopic(t, i)}/>
        <ChatMessages speed={this.state.messageSpeed} chosenTopics={this.state.chosenTopics} messagesOn={this.state.messagesOn} server={this.server} />
